...
Future<Uint8List> convertToBlackAndWhite(String assetPath) async {
  ...
  for (var y = 0; y < bwImage.height; y++) {
    for (var x = 0; x < bwImage.width; x++) {
      ...
      bwImage.setPixel(x, y, gray < 128 ? img.ColorInt8.rgb(0, 0, 0) : img.ColorInt8.rgb(255, 255, 255));
    }
  }
...
Future<Uint8List> convertForThermalPrinter(String assetPath) async {
  final ByteData bytes = await rootBundle.load(assetPath);
  final Uint8List originalImage = await convertToBlackAndWhite(assetPath);//bytes.buffer.asUint8List();
...
class CardImage extends StatelessWidget {
  ...
  Widget _buildImage() {
    ...
    if (imageSource is String) {
      if (imageSource.startsWith('data:image') || 
          _isBase64(imageSource)) {
        try {
          String base64String = imageSource.contains(',') 
              ? imageSource.split(',')[1] 
              : imageSource;
  ...
  Widget _buildPlaceholder() {
    return Container(
      ...
      child: placeholderWidget ?? const Center(
        child: Icon(
