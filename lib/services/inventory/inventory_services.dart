...
class InventoryServices {
  ...
  Future<InventoryServices> initialize() async {
    try {
      ...
      final productList = await dbQuery.fetchAllData('kiosk_product');
      final setList = await dbQuery.fetchAllData('set_product');
      productCatalog = {
      ...
      APP_LOGS.info('Set catalog preloaded: ${setCatalog!.length} items');
    } catch (e, stackTrace) {
      APP_LOGS.error('Error preloading data', e, stackTrace);
    }
    return this;
  }
  ...
  Future<List<int>> getKioskProductIds({bool isRetry = false}) async {
    try {
      ...
      if (productCatalog != null) {
        ...
        APP_LOGS.warning('Product catalog is empty even after refresh');
        return [];
      }
      ...
      APP_LOGS.warning('Failed to load product catalog after retry');
      return [];
    } catch (e) {
      APP_LOGS.error('Error retrieving product IDs: $e');
      return [];
    }
  }
    ...
    try {
      ...
      if (bulkData != null && bulkData.isNotEmpty) {
        for (final entry in bulkData.entries) {
          ...
          await dbQuery.updateData('kiosk_product', productId, updates);
          
          // Update cache if available
          if (productCatalog != null && productCatalog!.containsKey(productId)) {
      ...
      if (id != null && colName != null) {
        await dbQuery.updateCell('kiosk_product', id, colName, value);
      ...
      APP_LOGS.warning('No valid update parameters provided');
      return false;
    } catch (e) {
      APP_LOGS.error('Error updating product data: $e');
      return false;
    }
  }
  ...
  Future<bool> adjustProductQuantities({
    ...
    try {
      if (productCatalog == null) {
        await updateDataInVar();
        if (productCatalog == null) {
          APP_LOGS.error('Failed to load product catalog');
          return false;
        }
      }
      ...
      if (bulkData != null && bulkData.isNotEmpty) {
        ...
        for (final entry in bulkData.entries) {
          ...
          if (!productCatalog!.containsKey(productId)) {
            APP_LOGS.warning('Product ID $productId not found in catalog');
            continue;
          }
          ...
          if (data.containsKey('total_pieces_used')) {
            final currentValue = productCatalog![productId]!['total_pieces_used'] ?? 0;
      ...
      if (id != null && (totalStocks != null || totalPiecesUsed != null)) {
        ...
        if (updates.isNotEmpty) {
          final bulkUpdates = <String, dynamic>{
            id.toString(): updates,
          };
      ...
      APP_LOGS.warning('No valid adjustment parameters provided');
      return false;
    } catch (e, stackTrace) {
      APP_LOGS.error('Error adjusting product quantities', e, stackTrace);
      return false;
    }
  }
  ...
  Future<Map<String, dynamic>> parseSetString(
    ...
    try {
      if (setString.isEmpty) {
        APP_LOGS.error('Empty set configuration string provided');
        return {};
      }

      final List<String> segments = setString.split(',');
      if (segments.isEmpty) {
        APP_LOGS.error('Invalid set configuration format');
        return {};
      }
      ...
      for (int i = 1; i < segments.length; i++) {
        ...
        if (segment.endsWith('PIECE')) {
          ...
          if (id != null) {
            details.add({'ids': [id], 'max_qty': -1});
            fryValue = 1;
          } else {
            APP_LOGS.warning('Invalid ID format in FRY segment: $segment');
          }
          continue;
        }
        ...
        if (parts.length != 2) {
          APP_LOGS.warning('Skipping invalid format in segment: $segment');
          continue;
        }
        ...
        for (final spec in itemSpecs) {
          ...
          // Process category-based items - either containing # for spaces or pure alphabetic
            ...
            if (products.isNotEmpty) {
              ...
              APP_LOGS.warning('No products found for category: $category');
            }
          }
        }
        ...
        if (allMatchingIds.isNotEmpty) {
          ...
          APP_LOGS.warning('No matching products found for: $itemSpec');
        }
      }
      ...
      APP_LOGS.error('Error parsing set configuration', e, stackTrace);
      return {};
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getAllProductsAndSets() async {
    try {
      ...
      APP_LOGS.error('Error fetching products and sets: $e');
      return [];
    }
  }
